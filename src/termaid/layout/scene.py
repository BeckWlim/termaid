"""Shared terminal-cell layout surface for every diagram family.
...
class LayoutScene:
    ...
    def put_border_marker(self, row: int, col: int, marker: str, style: str = "arrow") -> bool:
        """Replace one straight border cell with its incident arrowhead.

        This explicit operation preserves protected labels, corners, and
        shape-specific symbols; ordinary edge placement cannot bypass them.
        """
        if not (0 <= row < self.height and 0 <= col < self.width):
            return False
        if self._grid[row][col] not in "─│━┃┄┆-|":
            return False
        self._grid[row][col] = marker
        self._style_grid[row][col] = style
        self._directions[row][col] = 0
        return True
